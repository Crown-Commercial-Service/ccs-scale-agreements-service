...
/**
 * Centralised error handling for application and container derived error
 * conditions.
 */
@ControllerAdvice
@RestController
...
public class GlobalErrorHandler implements ErrorController {
	...
	public ApiErrors handleValidationException(final Exception exception) {
		...
		ApiError apiError = new ApiError(HttpStatus.BAD_REQUEST.toString(), ERR_MSG_VALIDATION, exception.getMessage());
	...
	public ResponseEntity<ApiErrors> handleError(final HttpServletRequest request, final HttpServletResponse response) {

		Object exception = request.getAttribute("javax.servlet.error.exception");

		log.error("Unknown container/filter exception", exception);

		return ResponseEntity.badRequest().body(new ApiErrors(
				Arrays.asList(new ApiError(HttpStatus.INTERNAL_SERVER_ERROR.toString(), ERR_MSG_DEFAULT, ""))));
	}
